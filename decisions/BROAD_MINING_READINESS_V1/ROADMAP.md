...
|---|------|--------|--------------------------|
| 5 | **Autonomous driver** | GAP | `alpha idea run` = one idea per manual invocation; no loop pulls next idea. research-memory is write-then-query, not a director. `agent_factory` queue *forbids* `AUTONOMOUS_RESEARCH_RUNNING`. `ralph_driver` drives engineering phases, not ideas. |
| 2 | **Cross-year OOS aggregation** | GAP | `pooled_hypothesis.aggregate_pooled_metric` exists but is ORPHANED (only tests import it). `alpha idea run` is structurally single-partition (`--slice` scalar). Nothing fans an idea over `(instrument, year, horizon)` and pools. |
