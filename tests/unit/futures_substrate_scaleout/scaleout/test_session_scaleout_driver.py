...
CONFIG_PATH = "configs/features/scaleout/session_calendar_maintenance.json"
DATASET_ID_2024 = "dsv_databento_ohlcv_05404069799decb0"
DENSE_DATASET_ID_2024 = "dsv_databento_ohlcv_dense_2024_v1"
HASH_0 = "0" * 64
HASH_1 = "1" * 64
HASH_2 = "2" * 64


def test_session_scaleout_plan_groups_required_input_schemas() -> None:
    ...
    assert {record.unit.schema_id for record in summary.records} == {
...
def test_session_scaleout_execution_requires_all_input_locks_and_resumes(
    ...
    config = load_scaleout_config(CONFIG_PATH)
    registry_path = _registry_with_accepted_2024_inputs(tmp_path)
    ...
    def fake_executor(
        ...
        assert [dataset.dataset_version_id for dataset in unit.input_datasets] == [
            DATASET_ID_2024,
            DENSE_DATASET_ID_2024,
        ]
    ...
    ledger = (
        alpha_data_root
        / "materialization/futures_substrate_scaleout_v1/checkpoints/features/"
        "session_calendar_maintenance/completed_units.jsonl"
    )
...
def _registry_with_accepted_2024_inputs(tmp_path: Path) -> Path:
    registry_path = tmp_path / "registry" / "datasets.sqlite"
    policy = _acceptance_policy()
    locks = []
    for schema_id, dataset_version_id in (
        ("ohlcv_1m", DATASET_ID_2024),
        ("ohlcv_dense_research_grid", DENSE_DATASET_ID_2024),
    ):
        quality = _quality_report(dataset_version_id)
        coverage = _coverage_report(dataset_version_id)
...
def _acceptance_policy() -> dict[str, object]:
    return {
        ...
        "canonical_partition_schemas": {
            "ohlcv_1m": "ohlcv_1m",
            "ohlcv_dense_research_grid": "ohlcv_dense_research_grid",
        },
...
def _acceptance_evidence() -> dict[str, object]:
    return {
        "evidence_source": "synthetic_session_scaleout_unit_test",
        "row_count_sanity": {
            "status": ReportStatus.PASSING.value,
            "expected_bar_count": 1,
            "observed_bar_count": 1,
        },
        "gap_coverage": {"status": ReportStatus.PASSING.value, "gap_count": 0},
