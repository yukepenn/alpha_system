...
MODULE_NAME = "alpha_system.agent_factory.roles.label_engineer"
ROLE_ID = "label_engineer"


def test_module_imports_and_registers_label_engineer_once() -> None:
    ...
    assert registered == module.LABEL_ENGINEER_ROLE
...
def test_repeated_import_or_reload_does_not_duplicate_registration() -> None:
    ...
    assert registry_module.get(ROLE_ID) == reloaded.LABEL_ENGINEER_ROLE
...
def test_callable_tools_match_permission_matrix_and_exclude_forbidden_surfaces() -> None:
    ...
    assert permissions.write.allowed_scopes == ("label_spec.draft",)
...
def test_outputs_and_handoff_are_value_free_refs_only() -> None:
    ...
    assert "label_spec_ref or label_pack_ref" in role.handoff_format
