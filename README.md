...
`ALPHA_SYSTEM_V1` has completed phases `ASV1-P00` through `ASV1-P21`; the next planned phase is `ASV1-P22` Experiment Registry Hardening. The repo now includes the local-first harness baseline, core contracts, metadata registry, canonical 1-minute data layer, calendar/data-quality checks, factor and label foundations, factor diagnostics/reports, signal and strategy contracts, reference 1-minute backtest truth, cost/slippage semantics, position management, portfolio target/sizing, fast-path parity scaffolding, the bounded `alpha grid run` strategy grid MVP, and the survivor-gated bounded `alpha management grid` workflow.

The ASV1-P21 management-grid layer adds `src/alpha_system/experiments/management_grid.py`, `survivors.py`, `candidate_policy.py`, `overfit_controls.py`, `management_outputs.py`, `docs/MANAGEMENT_GRID_WORKFLOW.md`, `docs/SURVIVOR_POLICY.md`, and small configs under `configs/management/grid_examples/`. Safety boundaries are unchanged: local-first execution only, reference engine remains the single PnL truth, fast path is acceleration-only and parity-gated, bounded search, survivor-gated entry, no promotion decision by the grid, no broker/live/paper scope, no alpha/tradability claims, and explicit staging only.
