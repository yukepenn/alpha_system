...
`ALPHA_SYSTEM_V1` has completed phases `ASV1-P00` through `ASV1-P22`; the next planned phase is `ASV1-P23` ML and Factor Combination MVP. The repo now includes the local-first harness baseline, core contracts, metadata registry, canonical 1-minute data layer, calendar/data-quality checks, factor and label foundations, factor diagnostics/reports, signal and strategy contracts, reference 1-minute backtest truth, cost/slippage semantics, position management, portfolio target/sizing, fast-path parity scaffolding, the bounded `alpha grid run` strategy grid MVP, the survivor-gated bounded `alpha management grid` workflow, and hardened experiment registry modules for run records, artifact manifests, reproducibility metadata, promotion decisions, audit findings, failure visibility, and version references.

The ASV1-P22 registry layer adds `docs/EXPERIMENT_REGISTRY.md`, `docs/REPRODUCIBILITY_AUDIT.md`, and a read-only `alpha registry status` summary for audit, failed-run, and promotion-review state. Safety boundaries are unchanged: local-first execution only, SQLite remains local-only and uncommitted, reference engine remains the single PnL truth, fast path is acceleration-only and parity-gated, failed runs stay visible, promotion requires review, no broker/live/paper/deployment scope, no alpha/tradability claims, and explicit staging only.
