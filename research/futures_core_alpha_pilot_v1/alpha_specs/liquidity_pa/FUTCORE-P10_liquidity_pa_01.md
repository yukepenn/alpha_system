# FUTCORE-P10 Liquidity PA AlphaSpec 01
...
```json
{
  "alpha_spec_id": "aspec_1c1cfee8bedf55ced10a391e",
  "hypothesis_id": "hyp_001f84bc9c9e33365e1a7346",
  "target_instruments": [
    "ES",
    "NQ",
    "RTY"
  ...
  "data_assumptions": {
    ...
    "focus": "Prior high/low liquidity sweep event against a causal 20-bar range.",
    ...
    "draft_trigger_logic": "Trigger side is high_sweep when high >= prior_range_high_N20 + one tick and low_sweep when low <= prior_range_low_N20 - one tick; if both occur on the same completed bar the row is marked ambiguous and excluded from this draft.",
    "horizon_policy": {
  ...
  "exclusion_rules": [
    ...
    "Exclude bars that sweep both prior_range_high_N20 and prior_range_low_N20 in the same completed bar because trigger direction is ambiguous for this draft."
  ],
  "timestamp_assumptions": {
    "decision_timestamp": "Decision is after a completed bar whose high-side or low-side sweep crosses a prior 20-completed-bar reference level by at least one tick; close-back-inside, wick, displacement, compression, breakout, and failed-breakout flags are recorded as objective diagnostics but are not required for this trigger.",
    "available_ts_usage": {
  ...
  "expected_failure_modes": [
    ...
    "A broad prior high/low sweep can duplicate generic breakout or range-position exposure if the later critic cannot isolate stop-run behavior from ordinary range expansion."
  ],
  "promotion_criteria": [
    ...
    "Later diagnostics must separate high-side sweeps, low-side sweeps, both-side exclusions, and no-level exclusions by instrument and session view."
  ],
  "created_by": "Hypothesis Scout:rq.p10.liquidity_pa_drafting:prior_high_low_sweep",
