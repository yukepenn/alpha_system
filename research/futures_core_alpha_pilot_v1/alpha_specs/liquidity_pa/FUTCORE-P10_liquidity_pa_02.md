# FUTCORE-P10 Liquidity PA AlphaSpec 02
...
```json
{
  "alpha_spec_id": "aspec_df2d040e45564c259ef3de6d",
  "hypothesis_id": "hyp_575a2af3b0d0d6c6b7788722",
  "target_instruments": [
    "ES",
    "NQ",
    "RTY"
  ...
  "data_assumptions": {
    ...
    "focus": "Liquidity sweep followed by objective close-back-inside confirmation.",
    ...
    "draft_trigger_logic": "High-side setup requires high >= prior_range_high_N20 + one tick and close <= prior_range_high_N20 while close >= prior_range_low_N20 by at most one completed bar after the sweep; low-side setup mirrors the rule with low <= prior_range_low_N20 - one tick and close >= prior_range_low_N20.",
    "horizon_policy": {
  ...
  "exclusion_rules": [
    ...
    "Exclude sweeps whose close-back-inside deadline is missed by more than one completed bar because delayed returns inside the range are a separate failure mode."
  ],
  "timestamp_assumptions": {
    "decision_timestamp": "Decision is after a completed sweep bar or the immediately following completed bar when price has swept a prior 20-bar high or low by at least one tick and then closed back inside the same fixed prior range.",
    "available_ts_usage": {
  ...
  "expected_failure_modes": [
    ...
    "The close-back-inside rule may collapse into a generic mean-reversion trigger if the sweep distance and one-bar deadline do not materially constrain event selection."
  ],
  "promotion_criteria": [
    ...
    "Later diagnostics must report the sweep bar versus next-bar close-back-inside split and the count of deadline misses."
  ],
  "created_by": "Hypothesis Scout:rq.p10.liquidity_pa_drafting:close_back_inside",
