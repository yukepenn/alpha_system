# FUTCORE-P10 Liquidity PA AlphaSpec 04
...
```json
{
  "alpha_spec_id": "aspec_bc9bbcd07669384e51b1661f",
  "hypothesis_id": "hyp_84420e7ea623fad31b6d9b21",
  "target_instruments": [
    "ES",
    "NQ",
    "RTY"
  ...
  "data_assumptions": {
    ...
    "focus": "Post-sweep displacement as an objective impulsive reversal condition.",
    ...
    "draft_trigger_logic": "After a high-side sweep closes back inside, bearish displacement requires close_t_minus_1 - close_t >= 1.50 * prior20_median_true_range; after a low-side sweep closes back inside, bullish displacement requires close_t - close_t_minus_1 >= 1.50 * prior20_median_true_range; displacement must occur within two completed bars after the close-back-inside event.",
    "horizon_policy": {
  ...
  "exclusion_rules": [
    ...
    "Exclude displacement candidates when the prior20 median true range is missing, zero, or computed using the displacement bar itself."
  ],
  "timestamp_assumptions": {
    "decision_timestamp": "Decision is after a completed bar within two bars of a close-back-inside sweep when the reversal direction prints a displacement move of at least 1.50 times the previous 20-bar median true range.",
    "available_ts_usage": {
  ...
  "expected_failure_modes": [
    ...
    "Displacement can duplicate regime momentum/reversion or volatility-expansion exposure if the sweep prerequisite does not remain visible in later diagnostics."
  ],
  "promotion_criteria": [
    ...
    "Later diagnostics must report displacement magnitude buckets relative to prior20 median true range and the number of candidates that fail the two-bar deadline."
  ],
  "created_by": "Hypothesis Scout:rq.p10.liquidity_pa_drafting:sweep_reversal_displacement",
