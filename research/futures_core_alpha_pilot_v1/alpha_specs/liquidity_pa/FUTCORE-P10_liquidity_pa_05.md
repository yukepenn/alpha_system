# FUTCORE-P10 Liquidity PA AlphaSpec 05
...
```json
{
  "alpha_spec_id": "aspec_55c1351d04054c943c2c3721",
  "hypothesis_id": "hyp_beab96767e05d5b0964afe9b",
  "target_instruments": [
    "ES",
    "NQ",
    "RTY"
  ...
  "data_assumptions": {
    ...
    "focus": "Compression followed by objective breakout from a fixed range boundary.",
    ...
    "draft_trigger_logic": "Compression window qualifies when 8-bar high-low width <= 0.65 times the median of prior non-overlapping 8-bar widths from 60 completed bars; breakout is upside when close >= compression_range_high_N8 + one tick and downside when close <= compression_range_low_N8 - one tick after the compression window is fixed.",
    "horizon_policy": {
  ...
  "exclusion_rules": [
    ...
    "Exclude windows where the breakout bar is included in the compression-window boundary or where fewer than the prior 60 completed bars are available for the compression threshold."
  ],
  "timestamp_assumptions": {
    "decision_timestamp": "Decision is after a completed breakout bar that closes at least one tick beyond a fixed 8-bar compression boundary whose range width satisfies the compression threshold before breakout.",
    "available_ts_usage": {
  ...
  "expected_failure_modes": [
    ...
    "Compression breakout may duplicate the P09 regime compression-release draft unless later critique verifies this P10 draft is evaluated as a standalone objective PA trigger with fixed boundaries."
  ],
  "promotion_criteria": [
    ...
    "Later diagnostics must report compression-window eligibility, breakout side, boundary age, and breakout-close distance from the fixed boundary."
  ],
  "created_by": "Hypothesis Scout:rq.p10.liquidity_pa_drafting:compression_breakout",
