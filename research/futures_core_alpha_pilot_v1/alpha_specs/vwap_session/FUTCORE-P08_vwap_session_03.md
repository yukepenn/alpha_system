{
  "alpha_spec_id": "aspec_668088ee803855ce3e5617e6",
  "hypothesis_id": "hyp_98e469123f3374af96d39d00",
  "target_instruments": [
    "ES",
    "NQ",
    "RTY"
  ...
  "data_assumptions": {
    ...
    "formulation": "Evaluate predeclared absolute distance-to-running-VWAP buckets after completed bars, treating extreme distance as the event and avoiding final-session VWAP or final-session range inputs.",
    "input_pack_refs": {
    ...
    "duplicate_rejected_idea_awareness": "This is the generic distance-to-VWAP exposure that other reclaim/reject drafts may duplicate; FUTCORE-P12 should audit whether event drafts add information beyond this bucketed baseline.",
  ...
  "factor_inputs": [
    "Locked FeatureVersion fver_c365f971fe0c25d435eed9233bc7caa30a07dd761f43c3f288316b3a74894b0f base_ohlcv_rth_flag for segment filtering by available_ts.",
    "Locked FeatureVersion fver_17dfceba75a9883d2f78fdca07ebd4f59a40c5e98d9d5503d5a853395eedd978 base_ohlcv_session_minute for intraday bucket controls by available_ts.",
    "Existing OHLCVFeatureName.DISTANCE_TO_VWAP computed from running_vwap_so_far and completed OHLCV bars only.",
    "Existing OHLCVFeatureName.VWAP configured as running_vwap_so_far with session reset and no final-session VWAP input.",
    "Existing OHLCVFeatureName.ATR or rolling_range as optional normalization context if later StudySpec binding permits it."
  ],
  "label_references": [
  ...
  "exclusion_rules": [
    ...
    "Exclude buckets defined by sample quantiles computed on the evaluation window unless P14 records a fixed training-only binning rule.",
    "Exclude any normalization that uses final current-session high, low, range, or volume before the session closes."
  ],
  "timestamp_assumptions": {
    "decision_timing": "Decision is after a completed bar whose distance_to_running_vwap value and optional point-in-time normalization are available.",
    ...
    "bucket_timing": "Distance bucket assignment must be fixed before the label horizon begins and cannot depend on later intraday extrema.",
    "opening_range_availability": "Opening-range context is optional and only available after the configured opening window completes."
  },
  "cost_assumptions": {
  ...
  "expected_failure_modes": [
    ...
    "Predeclared distance buckets may be too coarse or too sparse across ES, NQ, and RTY for later split diagnostics.",
    "ATR or rolling-range normalization may introduce overlapping exposure with regime volatility filters from FUTCORE-P09."
  ],
  "promotion_criteria": [
    ...
    "Later diagnostics must compare bucketed distance exposure with reclaim and reject event drafts to identify redundant VWAP-family coverage."
  ],
