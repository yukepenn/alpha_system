{
  "alpha_spec_id": "aspec_4a0e170d68918d3c787db230",
  "hypothesis_id": "hyp_633007929fb843168fa3fb3a",
  "target_instruments": [
    "ES",
    "NQ",
    "RTY"
  ...
  "data_assumptions": {
    ...
    "formulation": "Evaluate completed-bar events where price exits the completed RTH opening range while its relation to running_vwap_so_far is predeclared as alignment or disagreement context.",
    "input_pack_refs": {
    ...
    "duplicate_rejected_idea_awareness": "Overlaps with future opening-range and breakout ideas; this draft is retained only as VWAP/session confluence and must not become a standalone price-action family draft.",
  ...
  "factor_inputs": [
    "Locked FeatureVersion fver_c365f971fe0c25d435eed9233bc7caa30a07dd761f43c3f288316b3a74894b0f base_ohlcv_rth_flag for RTH-only segmentation by available_ts.",
    "Locked FeatureVersion fver_17dfceba75a9883d2f78fdca07ebd4f59a40c5e98d9d5503d5a853395eedd978 base_ohlcv_session_minute for opening-window completion checks by available_ts.",
    "Existing OHLCVFeatureName.OPENING_RANGE with configured opening_range_minutes and availability only after the window completes.",
    "Existing OHLCVFeatureName.VWAP configured as running_vwap_so_far after each completed RTH bar.",
    "Existing StructureFeatureName.OPENING_RANGE_HIGH_DISTANCE and OPENING_RANGE_LOW_DISTANCE as optional objective distance context if later binding permits."
  ],
  "label_references": [
  ...
  "exclusion_rules": [
    ...
    "Exclude opening-range signals during the opening window because the completed range is not yet available.",
    "Exclude breakouts that rely on final RTH high, final RTH low, final RTH range, or final RTH volume before RTH close."
  ],
  "timestamp_assumptions": {
    "decision_timing": "Decision is after a completed post-opening-window RTH bar that exits the completed opening range and has running_vwap_so_far available.",
    ...
    "opening_range_availability": "Opening-range high and low become eligible only after the configured opening window has completed and received available_ts.",
    "running_vwap_context": "VWAP confluence uses running_vwap_so_far at the decision bar; final RTH VWAP cannot be used to classify the event."
  },
  "cost_assumptions": {
  ...
  "expected_failure_modes": [
    ...
    "Opening-range confluence may be duplicate exposure to FUTCORE-P10 breakout drafts unless VWAP relationship is the required conditioning variable.",
    "Changing the opening-window length could create variant explosion unless P14 fixes a small finite grid."
  ],
  "promotion_criteria": [
    ...
    "Later diagnostics must report opening-range availability and the fixed opening-window length before comparing any VWAP-aligned and VWAP-disagreed variants."
  ],
