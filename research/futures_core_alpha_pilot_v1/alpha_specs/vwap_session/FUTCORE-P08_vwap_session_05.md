{
  "alpha_spec_id": "aspec_ee701ec09d58e30b746c8e06",
  "hypothesis_id": "hyp_67323fbcaeca04d460bfd1c5",
  "target_instruments": [
    "ES",
    "NQ",
    "RTY"
  ...
  "data_assumptions": {
    ...
    "formulation": "Evaluate RTH decisions near completed overnight high or low while recording whether price is above or below running_vwap_so_far at the decision timestamp.",
    "input_pack_refs": {
    ...
    "duplicate_rejected_idea_awareness": "Overlaps with overnight high-low and sweep hypotheses; this draft is restricted to completed ETH context plus running RTH VWAP and must not use subjective level annotations.",
  ...
  "factor_inputs": [
    "Locked FeatureVersion fver_c365f971fe0c25d435eed9233bc7caa30a07dd761f43c3f288316b3a74894b0f base_ohlcv_rth_flag for RTH decision segmentation by available_ts.",
    "Locked FeatureVersion fver_17dfceba75a9883d2f78fdca07ebd4f59a40c5e98d9d5503d5a853395eedd978 base_ohlcv_session_minute for RTH timing controls by available_ts.",
    "Existing OHLCVFeatureName.OVERNIGHT_RANGE used only as completed ETH context after the overnight window ends.",
    "Existing OHLCVFeatureName.VWAP configured as running_vwap_so_far for the active RTH session.",
    "Existing StructureFeatureName.PRIOR_HIGH_DISTANCE and PRIOR_LOW_DISTANCE as optional objective distance context if later binding permits."
  ],
  "label_references": [
  ...
  "exclusion_rules": [
    ...
    "Exclude RTH_with_ETH_context rows where the overnight high or low is not completed before the RTH decision timestamp.",
    "Exclude intraday use of final RTH high, final RTH low, final RTH VWAP, final RTH range, or final RTH volume."
  ],
  "timestamp_assumptions": {
    "decision_timing": "Decision is after a completed RTH bar with completed overnight high-low context already available and running RTH VWAP available.",
    ...
    "overnight_high_low_availability": "Completed ETH high and low are available only after the overnight window ends; during ETH, only running overnight extrema may be used and must be named running.",
    "session_transition_handling": "The transition from ETH overnight to RTH must be split separately from RTH-only VWAP triggers in later diagnostics."
  },
  "cost_assumptions": {
  ...
  "expected_failure_modes": [
    ...
    "Completed overnight high-low context may be unavailable or sparse for some symbols after the P13 data-contract audit.",
    "The draft may duplicate liquidity sweep exposure if later StudySpec uses sweep flags rather than distance-to-completed-overnight-extreme context."
  ],
  "promotion_criteria": [
    ...
    "Later diagnostics must report RTH_with_ETH_context separately from RTH_only so completed overnight context is not blended with ordinary RTH VWAP exposure."
  ],
